...
namespace MetroDemo
{
    public class AccentColorMenuData
    {
        ...
        {
            get { return this.changeAccentCommand ?? (changeAccentCommand = new SimpleCommand {
                CanExecuteDelegate = x => true, ExecuteDelegate = x => this.DoChangeTheme(x) }); }
        }

        protected virtual void DoChangeTheme(object sender)
        {
            var theme = ThemeManager.DetectAppStyle(ApplicationX.Current);
            var accent = ThemeManager.GetAccent(this.Name);
            ThemeManager.ChangeAppStyle(ApplicationX.Current, accent, theme.Item1);
        }
    }
    ...
    public class AppThemeMenuData : AccentColorMenuData
    {
        protected override void DoChangeTheme(object sender)
        {
            ...
            ThemeManager.ChangeAppStyle(ApplicationX.Current, theme.Item2, appTheme);
        }
    }
    ...
    public class MainWindowViewModel : INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        public MainWindowViewModel(IDialogCoordinator dialogCoordinator)
        {
            ...
            this.AccentColors = ThemeManager.Accents
                                            .Select(a => new AccentColorMenuData() { Name = a.Name,
                                                ColorBrush = a.Resources["AccentColorBrush"] as Brush })
                                            .ToList();

            // create metro theme color menu items for the demo
            this.AppThemes = ThemeManager.AppThemes
                                           .Select(a => new AppThemeMenuData() { Name = a.Name,
                                               BorderColorBrush = a.Resources["BlackColorBrush"] as Brush, ColorBrush = a.Resources["WhiteColorBrush"] as Brush })
                                           .ToList();
            ...
            FlipViewImages = new string[] { "http://trinities.org/blog/wp-content/uploads/red-ball.jpg",
                "http://savingwithsisters.files.wordpress.com/2012/05/ball.gif" };
            ...
            get
            {
                return this.textBoxButtonCmd ?? (this.textBoxButtonCmd = new SimpleCommand
                {
                    ...
                    ExecuteDelegate = async x =>
                    {
                        if (x is TextBox)
                        {
                            await ((MetroWindow) ApplicationX.Current.MainWindow).ShowMessageAsync("TextBox Button was clicked!",
                                                                                                    string.Format("Text: {0}", ((TextBox) x).Text));
                        }
                        else if (x is PasswordBox)
                        {
                            await ((MetroWindow) ApplicationX.Current.MainWindow).ShowMessageAsync("PasswordBox Button was clicked!",
            ...
            get
            {
                return this.textBoxButtonCmdWithParameter ?? (this.textBoxButtonCmdWithParameter = new SimpleCommand
                {
                    ...
                    ExecuteDelegate = async x =>
                    {
                        if (x is String)
                        {
                            await ((MetroWindow) ApplicationX.Current.MainWindow).ShowMessageAsync("TextBox Button with parameter was clicked!",
        ...
        public ICommand SingleCloseTabCommand
        {
            get
            {
                return this.singleCloseTabCommand ?? (this.singleCloseTabCommand = new SimpleCommand
                {
                    ...
                    ExecuteDelegate = async x =>
                    {
                        await ((MetroWindow) ApplicationX.Current.MainWindow).ShowMessageAsync("Closing tab!", string.Format("You are now closing the '{0}' tab", x));
                    }
                });
            }
        }
